...
<body>
    ...
    <div class="container">
        ...
        try {
            ...
            $query = "SELECT username, password, first_name, last_name, gender, birthdate, email FROM customers WHERE username = ? LIMIT 0,1";
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo "<a href='c_update.php?username={$c_username}' class='btn btn-primary m-r-1em'>Edit</a>";
        ...
        if ($_POST) {
            ...
            $query = "SELECT email FROM customers WHERE email = ? AND username <> ?";
            $stmt = $con->prepare($query);
            $stmt->bindParam(1, $mail);
            $stmt->bindParam(2, $c_username);
            $stmt->execute();
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
